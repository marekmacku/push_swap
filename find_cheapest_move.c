...
static void set_move_costs(t_move *move, int b_cost, int a_cost, int total_cost)
{
    move->b_cost = b_cost;
    move->a_cost = a_cost;
...
static void check_and_update_move(t_move_context *ctx, t_element_info *elem)
{
    ...
    calc_dir_cost(elem->b_pos, ctx->stack_b->size, &rot_data.b_dir, &rot_data.b_cost);
    calc_dir_cost(target_pos, ctx->stack_a->size, &rot_data.a_dir, &rot_data.a_cost);
    ...
    if (total_cost < *ctx->best_cost)
    {
        *ctx->best_cost = total_cost;
        ...
        set_move_costs(ctx->move, rot_data.b_cost, rot_data.a_cost, total_cost);
